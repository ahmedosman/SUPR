...
def verts_core(pose, v, J, weights, kintree_table,batch_size,num_joints):
    ...
    rest_shape_h = tf.concat([v, tf.ones([batch_size, 10475, 1], dtype=tf.float32)], axis=-1)
...
class SUPR(object):
    ...
    def __call__(self,pose,betas,trans):
        ...
        self.shapedirs      = tf.constant(self.model['shapedirs'][:,:,:self.num_betas],dtype=dtype)
        self.weights        = tf.constant(self.model['weights'],dtype=dtype)
        self.kintree_table  = self.model['kintree_table'].astype(np.int32)
        self.f = self.model['f']
        tf_v_template = tf.constant(np.tile(self.model['v_template'],[batch_size,1,1]),dtype=dtype)
